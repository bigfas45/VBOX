...
router.post(
    ...
    async function (req, res) {
        try {
            var hash = crypto.createHmac('sha512', paystackSecretKey).update(JSON.stringify(req.body)).digest('hex');
            ...
            const { data, status, message } = await PaymentService.confirmTransaction(reference, paymentStatus, req.body);
            if (status === 'failed') {
                await LogService.create({
                    payload: req.body,
                    description: message,
